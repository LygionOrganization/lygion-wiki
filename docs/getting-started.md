...
!!! warning "禁止直接将多个新设备同时接入总线"
    大多数总线设备出厂默认 ID 可能相同，例如默认 ID 为 `1`。  
    如果多个相同 ID 的设备同时接入总线，会导致通信冲突，表现为搜索异常、控制（反馈）异常或设备无响应。  
...
![TTL Adapter USB Connection](assets/images/g02.png){ .img-rounded }
...
![TTL Adapter USB Connection](assets/images/g03.png){ .img-rounded }
...
如果需要使用多组电源，可以断开不同供电组之间的 V+，但需要保留 GND 和 S 的连接。
...
Windows 用户需要先安装 [Python](https://www.python.org/downloads/windows/)。
...
进入 SDK 目录后，建议创建并激活 Python 虚拟环境。
...
对于 ESP32、STM32、Arduino 等嵌入式设备用户，可以使用 Lygion C++ SDK 或 Arduino 框架示例进行开发。
